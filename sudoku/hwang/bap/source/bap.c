...
See bap.txt for a discussion of issues and implementation decisions.
The format of problem files is described there.
...
files in the ~/reason/squares/gen_gen/text directory.

The explanations this gives are not as complete or clear as they
should be. This should print an explanation each time a ship is
found. Currently locate_ships is not reporting progress.

The method of locating a ship is weird. First, when it is realized
that a chain of ship parts makes a ship, water is placed at the ends
of the ship. Later, the ship is identified by its length.

The executable can handle any number of ships of the four types and
can handle any size rectangular grid from 1-by-1 to 20-by-20. Larger
grids can be handled by changing some of the #defines.

The puzzle statement can have water and/or known ship parts in it,
but not unknown ship parts.
...
int record_progress(char * reason, char * action, struct bap_world * world);
...
int try_cross(struct bap_world * world);
int try_cross_col(struct bap_world * world);
int try_cross_col_fill_col(int col, int row1, int row2,
  struct bap_world * world);
int try_cross_col_fill_row(int row, int number_yes_cols, int * yes_cols,
  struct bap_world * world);
int try_cross_col_one(int number_test_cols, int * test_cols, int row1,
  int row2, struct bap_world * world);
int try_cross_row(struct bap_world * world);
int try_cross_row_fill_col(int col, int number_yes_rows, int * yes_rows,
  struct bap_world * world);
int try_cross_row_fill_row(int row, int col1, int col2,
  struct bap_world * world);
int try_cross_row_one(int number_test_rows, int * test_rows, int col1,
  int col2, struct bap_world * world);
...
int try_full_but_two(struct bap_world * world);
int try_full_but_two_col(int col, struct bap_world * world);
int try_full_but_two_row(int row, struct bap_world * world);
 ...
 struct bap_world * world)  /* puzzle model */
{
  ...
  if (total_assigned > 0)
    {
      world->progress SET_TO 1;
      if (total_assigned IS 1)
	IFF(record_progress("assign battleship", "to", world));
      else
	IFF(record_progress("assign battleships", "to", world));
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  if (total_assigned > 0)
	...
	IFF(record_progress("assign cruisers", "to", world));
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  if (total_assigned > 0)
	...
	IFF(record_progress("assign destroyers", "to", world));
...
Called By:
  ...
  place_ships_col
  place_ships_row
  try_cross_col_fill_row
  try_cross_row_fill_col
  try_full
  try_full_but_one_col
  try_full_but_one_row
  try_full_but_two_col
  try_full_but_two_row
  ...
  if (world->progress)
    {
      ...
      IFF(record_progress("extend horizontally", "ship middle at", world));
    }
  return OK;
}
  ...
  if (world->progress)
    {
      ...
      world->rows_in[0] SET_TO row;
      world->cols_in[0] SET_TO col;
      IFF(record_progress("extend vertically", "ship middle at", world));
    }
  return OK;
}
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  IFF(record_progress
      ("all blanks must be water", "fill all blanks with water in", world));
  return OK;
}
 ...
 struct bap_world * world) /* puzzle model         */
{
  ...
  if (the_guess->size IS 1)
    ...
    {
      strcpy(buffer, "battleship is in column at");
      IFF(insert_guess_square(row, col, '^', world));
      IFF(insert_guess_square((row + 1), col, '+', world));
  ...
  world->rows_in[0] SET_TO row;
  world->number_in_rows SET_TO 1;
  IFF(record_progress("guess", buffer, world));
  return OK;
}
 ...
 struct bap_world * world) /* puzzle model         */
{
  ...
  if (the_guess->size IS 1)
    ...
    {
      strcpy(buffer, "destroyer is in row at");
      IFF(insert_guess_square(row, col, '<', world));
      IFF(insert_guess_square(row, (col + 1), '>', world));
    }
  else if (the_guess->size IS 3)
    {
      strcpy(buffer, "cruiser is in row at");
      IFF(insert_guess_square(row, col, '<', world));
      IFF(insert_guess_square(row, (col + 1), '+', world));
    ...
    {
      strcpy(buffer, "battleship is in row at");
      IFF(insert_guess_square(row, col, '<', world));
      IFF(insert_guess_square(row, (col + 1), '+', world));
      IFF(insert_guess_square(row, (col + 2), '+', world));
...
Called By:
  ...
  try_cross_col_fill_col
  try_cross_row_fill_row
  try_full_but_two_col
  try_full_but_two_row
  try_square_blank
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  IFF(record_progress
      ("place battleships", "mark ship parts and/or water in", world));
  return OK;
}
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  IFF(record_progress
      ("place cruisers", "mark ship parts and/or water in", world));
  return OK;
}
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  IFF(record_progress("place destroyers", "ship parts and/or water in", world));
  return OK;
}
...
int place_ships_col_water( /* ARGUMENTS                                     */
 ...
 int * placed,             /* set to one if any water placed                */
 struct bap_world * world) /* puzzle model                                  */
{
...
cases are marked with an X.  Both placements must be legal. In addition,
this calls place_ships_row_water to mark water in the row.
...
int record_progress(       /* ARGUMENTS                     */
 char * reason,            /* text giving reason for action */
 char * action,            /* text describing action taken  */
 struct bap_world * world) /* puzzle model                  */
{
  ...
  if (world->progress ISNT 0)
    {
      where SET_TO world->logic[world->logic_line];
      numb SET_TO sprintf(where, "%s: %s ", reason, action);
...
Returned Value: int
  ...
  1. One of the following functions returns ERROR:
     ...
     place_destroyers, print_bap, try_cross, try_full, try_full_but_one,
     try_guessing, try_square_blank, verify_squares.
...
16. Look at each blank square, one at a time, and determine if it can
    ...
    (try_square_blank).
17. Look for the situation described in documentation of try_cross
    and mark ship parts (and water) if possible.
18. If done, print the puzzle and the explanation of how it was solved.
...
which calls solve_problem recursively. As a result of the recursive
call, multiple solutions can be found.
...
Returned Value: int
  If any of the following errors occur, this returns ERROR.
  Otherwise, it returns OK.
  1. try_cross_row returns ERROR.
  2. try_cross_column returns ERROR.

Called By:  solve_problem

Side Effects:

For rows, this checks whether there are two or more rows in each of
which one more ship part is needed. If so (call the number of such
rows N), it checks whether there are two columns that have blanks
in the test rows. In this case, the two columns must have a total
of at least N blanks in the test rows, since at most one of the
two columns can have a ship part in each test row. Thus, if
[N + the total number of ship parts needed in the two columns] equals
[the total number of blanks in those two columns], then:
(1) All the blanks in those two columns that are not in the
    test rows must have ship parts in them.
(2) Each of the remaining ship parts needed in the two columns must be
    in one of the two rows, so any other blank in any of the test
    rows must be water.
The function marks the ship parts and the water just described, if
the conditions are met.

For columns, this performs analogously.

This returns as soon as any case it is checking for is found and
the grid is marked.

Notes:

A simple example of this is the following, which shows two rows and
two columns on the left. There are only two ways these rows and
columns can be completed, as shown in the middle two figures.  In both
cases, all the squares in the two rows that are not in the two columns
must be water and are so marked, as shown on the right.
...
A slightly more complex example of this is the followin., Again, the
middle two figures show all the possible ways to mark the two columns,
and the figure on the right shows what the function does. In this
case the function marks ship parts in the first row, as well as
marking water in the other two rows.
...
Here is a more complex real example (from gam0411_6.pro). In the
figure on the left, there are three rows each of which needs one more
ship part, so N = 3. Looking at columns 1 and 5, (a) each of the test
rows has blanks in those columns, (b) those columns require a total of
8 more ship parts and (c) those columns contain 11 blank squares. Since
8+3 = 11, the rule applies, so the function marks the grid as shown
on the right.
...
int try_cross(             /* ARGUMENTS    */
 struct bap_world * world) /* puzzle model */
{
  const char * name SET_TO "try_cross";

  world->number_in_rows SET_TO 0;
  world->number_in_cols SET_TO 0;
  IFF(try_cross_row(world));
  if (NOT (world->progress))
    IFF(try_cross_col(world));
  return OK;
}
...
Returned Value: int
  If any of the following errors occur, this returns ERROR.
  Otherwise, it returns OK.
  1. try_cross_col_one returns ERROR.

Called By:  try_cross

Side Effects: See documentation of try_cross. This handles the case
when the strips needing one more ship part are columns. The two
crossing strips are rows.

Notes:

This counts the number of columns that need one more ship part.  If
there are fewer than two, it does nothing further.  If there are two
or more, it calls try_cross_col_one for all pairs of rows, except that
it stops as soon as a pair is found for which one or more squares can
be marked as ship or water.

*/

int try_cross_col(         /* ARGUMENTS     */
 struct bap_world * world) /* puzzle model  */
{
  const char * name SET_TO "try_cross_col";
  int number_test_cols;           /* number of columns needing 1 ship part  */
  int test_cols[MAX_NUMBER_COLS]; /* indexes of columns needing 1 ship part */
  int row1;                       /* first row to test                      */
  int row2;                       /* second row to test                     */
  int k;                          /* counter for columnss                   */

  number_test_cols SET_TO 0;
  for (k SET_TO 0; k < world->number_cols; k++)
    {
      if ((world->cols[k].aim - world->cols[k].got) IS 1)
	test_cols[number_test_cols++] SET_TO k;
    }
  if (number_test_cols > 1)
    {
      for (row1 SET_TO 0; row1 < (world->number_rows - 1); row1++)
	{
	  if (world->progress)
	    break;
	  for (row2 SET_TO (row1 + 1); row2 < world->number_rows; row2++)
	    {
	      if (world->progress)
		break;
	      IFF(try_cross_col_one
		  (number_test_cols, test_cols, row1, row2, world));
	    }
	}
    }
  return OK;
...
Called By:  try_cross_col_one

Side Effects: See documentation of try_cross. If the conditions have
been met, this function is called. It puts water in each blank in the
given column that is not in one of the two rows. If any square is
filled with water, the function also records the column number.

*/

int try_cross_col_fill_col( /* ARGUMENTS            */
 int col,                   /* index of column      */
 int row1,                  /* index of first row   */
 int row2,                  /* index of second row  */
 struct bap_world * world)  /* puzzle model         */
{
  const char * name SET_TO "try_cross_col_fill_col";
  int filled_one; /* flag set if water written */
  int n;          /* row index */

  filled_one SET_TO 0;
  for (n SET_TO 0; n < world->number_rows; n++)
    {
      if ((n ISNT row1) AND (n ISNT row2) AND (square(n, col, world) IS ' '))
	{
	  IFF(make_water(n, col, world));
	  filled_one SET_TO 1;
	}
    }
  if (filled_one)
...
Side Effects: See documentation of try_cross. This fills each blank
square in the selected row that is not in any of the yes_cols with an
X. If any square is filled, it also records the row number.

Notes:

In the first if statement, if the column being examined is a yes_col, it
is skipped and the yes_col counter, m, is increased by one. Then, if
all the yes_cols have been encountered (in which case m is
number_yes_cols), m is reset to 0 so that the indexing of yes_cols is
in bounds the next time around the loop (and n will not equal
yes_cols[m]).

*/

int try_cross_col_fill_row( /* ARGUMENTS                   */
 int row,                   /* index of row                */
 int number_yes_cols,       /* number of yes_cols          */
 int * yes_cols,            /* array of indexes of columns */
 struct bap_world * world)  /* puzzle model                */
{
  const char * name SET_TO "try_cross_col_fill_row";
  int k;          /* column counter        */
  int m;          /* yes_cols counter      */
  int filled_one; /* flag set if X written */
  ...
  for (k SET_TO 0; k < world->number_cols; k++)
	...
	{
	  m++;
	  if (m IS number_yes_cols)
	    m SET_TO 0;
	}
      else if (square(row, k, world) IS ' ')
	{
	  IFF(enx_square(row, k, world));
	  filled_one SET_TO 1;
	}
...
Returned Value: int
  If any of the following errors occur, this returns ERROR.
  Otherwise, it returns OK.
  1. There are not enough blanks available in row1 and row2 to fill
     the rows with the required number of ship parts: "Puzzle has no answer".
  2. try_cross_col_fill_row returns ERROR.
  3. record_progress returns ERROR.

Called By:  try_cross_col

Side Effects: See documentation of try_cross. This handles the
case when the strips needing one more ship part are columns and two
specific rows have been picked to test.

Notes:

This function is unusual in that it may mark ship parts and/or water.
Hence, it calls record progress twice, once for marking ship parts
in rows and once for marking water in columns.

*/

int try_cross_col_one(     /* ARGUMENTS                        */
 int number_test_cols,     /* number of test columns           */
 int * test_cols,          /* array of indexes of test columns */
 int row1,                 /* index of first row               */
 int row2,                 /* index of second row              */
 struct bap_world * world) /* puzzle model                     */
{
  const char * name SET_TO "try_cross_col_one";
  int number_yes_cols;
  int yes_cols[MAX_NUMBER_COLS];
  int needed;
  int available;
  int k;
  int save_progress;

  number_yes_cols SET_TO 0;
  for (k SET_TO 0; k < number_test_cols; k++)
    {
      if ((world->squares[row1][test_cols[k]] IS ' ') AND
	  (world->squares[row2][test_cols[k]] IS ' '))
	yes_cols[number_yes_cols++] SET_TO test_cols[k];
    }
  if (number_yes_cols > 1)
    {
      needed SET_TO (world->rows[row1].aim + world->rows[row2].aim);
      available SET_TO
	((world->rows[row1].dry + world->rows[row2].dry) - number_yes_cols);
      CHK((needed > available), "Puzzle has no answer");
      if (needed IS available)
	{
	  IFF(try_cross_col_fill_row(row1, number_yes_cols, yes_cols, world));
	  IFF(try_cross_col_fill_row(row2, number_yes_cols, yes_cols, world));
	  IFF(record_progress
	      ("cross col test", "fill some blanks with ship in", world));
	  world->number_in_rows SET_TO 0;
	  save_progress SET_TO world->progress;
	  world->progress SET_TO 0;
	  for (k SET_TO 0; k < number_yes_cols; k++)
	    {
	      IFF(try_cross_col_fill_col(yes_cols[k], row1, row2, world));
	    }
	  IFF(record_progress
	      ("cross col test", "fill some blanks with water in", world));
	  if (save_progress)
	    world->progress SET_TO 1;
	}
    }
...
Returned Value: int
  If any of the following errors occur, this returns ERROR.
  Otherwise, it returns OK.
  1. try_cross_row_one returns ERROR.

Called By:  try_cross

Side Effects: See documentation of try_cross. This handles the
case when the strips needing one more ship part are rows. The two
crossing strips are columns.

Notes:

This counts the number of rows that need one more ship part.  If
there are fewer than two, it does nothing further.  If there are two
or more, it calls try_cross_row_one for all pairs of columns, except that
it stops as soon as a pair is found for which one or more squares can
be marked as ship or water.

*/

int try_cross_row(         /* ARGUMENTS    */
 struct bap_world * world) /* puzzle model */
{
  const char * name SET_TO "try_cross_row";
  int number_test_rows;           /* number of rows needing 1 ship part  */
  int test_rows[MAX_NUMBER_ROWS]; /* indexes of rows needing 1 ship part */
  int col1;                       /* first column to test                */
  int col2;                       /* second column to test               */
  int n;                          /* counter for rows                    */

  number_test_rows SET_TO 0;
  for (n SET_TO 0; n < world->number_rows; n++)
    {
      if ((world->rows[n].aim - world->rows[n].got) IS 1)
	test_rows[number_test_rows++] SET_TO n;
    }
  if (number_test_rows > 1)
    {
      for (col1 SET_TO 0; col1 < (world->number_cols - 1); col1++)
	{
	  if (world->progress)
	    break;
	  for (col2 SET_TO (col1 + 1); col2 < world->number_cols; col2++)
	    {
	      if (world->progress)
		break;
	      IFF(try_cross_row_one
		  (number_test_rows, test_rows, col1, col2, world));
	    }
	}
    }
  return OK;
...
Called By:  try_cross_row_one

Side Effects: This fills each blank square in column col that is
not in any of the yes_rows with an X. If any square is filled, it
also records the column number.

Notes:

In the first if statement, if the row being examined is a yes_row, it
is skipped and the yes_row counter, m, is increased by one. Then, if
all the yes_rows have been encountered (in which case m is
number_yes_rows), m is reset to 0 so that the indexing of yes_rows is
in bounds the next time around the loop (and n will not equal
yes_rows[m]).

*/

int try_cross_row_fill_col( /* ARGUMENTS                */
 int col,                   /* index of column          */
 int number_yes_rows,       /* number of yes_rows       */
 int * yes_rows,            /* array of indexes of rows */
 struct bap_world * world)  /* puzzle model             */
{
  const char * name SET_TO "try_cross_row_fill_col";
  int n;          /* row counter           */
  int m;          /* yes_rows counter      */
  int filled_one; /* flag set if X written */
  ...
  for (n SET_TO 0; n < world->number_rows; n++)
	...
	{
	  m++;
	  if (m IS number_yes_rows)
	    m SET_TO 0;
	}
      else if (square(n, col, world) IS ' ')
	{
	  IFF(enx_square(n, col, world));
	  filled_one SET_TO 1;
	}
    }
  if (filled_one)
...
Side Effects: See documentation of try_cross. If the conditions have
been met, this function is called. It puts water in each blank in the
given row that is not in one of the two columns. If any square is
filled with water, the function also records the row number.

*/

int try_cross_row_fill_row( /* ARGUMENTS                 */
 int row,                   /* index of row              */
 int col1,                  /* index of first column     */
 int col2,                  /* index of second column    */
 struct bap_world * world)  /* puzzle model              */
{
  const char * name SET_TO "try_cross_row_fill_row";
  int filled_one; /* flag set if water written */
  int k;          /* column index */
  ...
  for (k SET_TO 0; k < world->number_cols; k++)
    {
      if ((k ISNT col1) AND (k ISNT col2) AND (square(row, k, world) IS ' '))
	{
	  IFF(make_water(row, k, world));
	  filled_one SET_TO 1;
	}
    }
  if (filled_one)
...
Returned Value: int
  If any of the following errors occur, this returns ERROR.
  Otherwise, it returns OK.
  1. There are not enough blanks available in col1 and col2 to fill the
     columns with the required number of ship parts: "Puzzle has no answer".
  2. try_cross_row_fill_col returns ERROR.
  3. record_progress returns ERROR.

Called By:  try_cross_row

Side Effects: See documentation of try_cross. This handles the
case when the strips needing one more ship part are rows and two
specific columns have been picked to test.

Notes:

This function is unusual in that it may mark ship parts and/or water.
Hence, it calls record progress twice, once for marking ship parts
in columns and once for marking water in rows.

*/

int try_cross_row_one(     /* ARGUMENTS                     */
 int number_test_rows,     /* number of test rows           */
 int * test_rows,          /* array of indexes of test rows */
 int col1,                 /* index of first column         */
 int col2,                 /* index of second column        */
 struct bap_world * world) /* puzzle model                  */
{
  const char * name SET_TO "try_cross_row_one";
  int number_yes_rows;
  int yes_rows[MAX_NUMBER_ROWS];
  int needed;
  int available;
  int n;
  int save_progress;

  number_yes_rows SET_TO 0;
  for (n SET_TO 0; n < number_test_rows; n++)
    {
      if ((world->squares[test_rows[n]][col1] IS ' ') AND
	  (world->squares[test_rows[n]][col2] IS ' '))
	yes_rows[number_yes_rows++] SET_TO test_rows[n];
    }
  if (number_yes_rows > 1)
    {
      needed SET_TO (world->cols[col1].aim + world->cols[col2].aim);
      available SET_TO
	((world->cols[col1].dry + world->cols[col2].dry) - number_yes_rows);
      CHK((needed > available), "Puzzle has no answer");
      if (needed IS available)
	{
	  IFF(try_cross_row_fill_col(col1, number_yes_rows, yes_rows, world));
	  IFF(try_cross_row_fill_col(col2, number_yes_rows, yes_rows, world));
	  IFF(record_progress
	      ("cross row test", "fill some blanks with ship in", world));
	  world->number_in_cols SET_TO 0;
	  save_progress SET_TO world->progress;
	  world->progress SET_TO 0;
	  for (n SET_TO 0; n < number_yes_rows; n++)
	    {
	      IFF(try_cross_row_fill_row(yes_rows[n], col1, col2, world));
	    }
	  IFF(record_progress
	      ("cross row test", "fill some blanks with water in", world));
	  if (save_progress)
	    world->progress SET_TO 1;
	}
    }
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  int n;            /* index for rows      */
  int k;            /* index for columns   */
  int filled_one;
  ...
  IFF(record_progress
      ("all blanks must be ship", "fill all blanks with ship in", world));
  return OK;
}
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  IFF(record_progress
      ("all blanks but one must be ship", "fill some blanks in", world));
  return OK;
}
...
Returned Value: int
  ...
  2. enx_square returns ERROR.
  3. There is no legal way to fill the column: "Puzzle has no answer".
 ...
 struct bap_world * world) /* puzzle model  */
{
  ...
  char test_array[MAX_TEST];     /* array containing test column           */
  char * test;                   /* test column                            */
  int n;                         /* row index, reused                      */
  int rows;                      /* number of rows in puzzle               */
  int row;                       /* test row to put water in               */
  int filled_one;                /* flag indicating square set in column   */
  int is_legal;                  /* flag set by legal_col, 1=col is OK     */
  int found_legal;               /* flag set to 1 if legal column is found */
  char setting;                  /* character in a square                  */

  filled_one SET_TO 0;
  found_legal SET_TO 0;
  test_array[0] SET_TO '.';
  rows SET_TO world->number_rows;
  test SET_TO (test_array + 1);
  test[rows] SET_TO '.';
  for (row SET_TO 0; row < world->number_rows; row++)
    {
	...
	continue;
      for (n SET_TO 0; n < world->number_rows; n++)
	{
	  setting SET_TO square(n, col, world);
	  test[n] SET_TO
	    ((n IS row) ? '.' : (setting IS ' ') ? 'Y' : setting);
	}
      IFF(legal_col(test, col, &is_legal, world));
      if (is_legal)
	found_legal SET_TO 1;
      else
	{
	  IFF(enx_square(row, col, world));
	  filled_one SET_TO 1;
	}
    }
  if (filled_one)
...
Returned Value: int
  ...
  2. enx_square returns ERROR.
  3. There is no legal way to fill the row: "Puzzle has no answer".
...
If there is only one legal way to fill the row, the blank that must be
water could also be marked here. The filled_one variable could be used
to identify the column that gets the water and the fact that only one
legal way was found. Since the row's aim will be met in this case,
fill_bap will put water in that column next time it runs, so it does
not matter much that it is not being done here.
 ...
 struct bap_world * world) /* puzzle model */
{
  ...
  char test_array[MAX_TEST]; /* array containing test row          */
  char * test;               /* test row                           */
  int n;                     /* column index, reused               */
  int cols;                  /* number of columns in puzzle        */
  int col;                   /* test column to put water in        */
  int filled_one;            /* flag indicating square set in row  */
  int is_legal;              /* flag set by legal_row, 1=row is OK */
  int found_legal;
  char setting;
  ...
  filled_one SET_TO 0;
  found_legal SET_TO 0;
  for (col SET_TO 0; col < cols; col++)
    {
      if (world->squares[row][col] ISNT ' ')
	continue;
      for (n SET_TO 0; n < cols; n++)
	{
	  setting SET_TO square(row, n, world);
	  test[n] SET_TO
	    ((n IS col) ? '.' : (setting IS ' ') ? 'Y' : setting);
	}
      IFF(legal_row(test, row, &is_legal, world));
      if (is_legal)
	found_legal SET_TO 1;
      else
	{
...
Returned Value: int
  If any of the following errors occur, this returns ERROR.
  Otherwise, it returns OK.
  1. try_full_but_two_row returns ERROR.
  2. try_full_but_two_col returns ERROR.
  3. record_progress returns ERROR.

Called By:  solve_problem

Side Effects: This reads the rows and columns looking for ones for
which the aim number is two less than the dry number (meaning two of
the dry squares should be water and the rest ship). For each such
strip, repeatedly, a copy of the strip is made with two blanks filled
with water and the rest filled with ship. The copy is tested for being
legal. Any position in the strip that was blank but is filled with
ship in every legal copy of the strip must be ship, so an X is placed
in that position.  Also, any position in the strip that was blank but
is filled with water in every legal copy of the strip must be water,
so a . is placed in that position. A line of explanation is written if
any X or any water is marked.

This stops as soon as any progress is made because the tests are
computationally expensive.

*/

int try_full_but_two(      /* ARGUMENTS    */
 struct bap_world * world) /* puzzle model */
{
  const char * name SET_TO "try_full_but_two";
  int row;
  int col;
  ...
  for (row SET_TO 0; row < world->number_rows; row++)
    {
      if (world->progress)
	break;
      if (world->rows[row].dry IS (world->rows[row].aim + 2))
	IFF(try_full_but_two_row(row, world));
    }
  if (NOT (world->progress))
    for (col SET_TO 0; col < world->number_cols; col++)
      {
	if (world->progress)
	  break;
	if (world->cols[col].dry IS (world->cols[col].aim + 2))
	  IFF(try_full_but_two_col(col, world));
      }
  IFF(record_progress
      ("all blanks but two must be ship", "fill some blanks in", world));
  return OK;
}
...
Called By:  try_full_but_two

Side Effects: This is called if all the blanks but two in a column must be
filled with ship parts. The remaining two must be filled with water.

The idea is to find if any blank squares of the column are ship in all
legal coverings of the column and if any blank squares are water in all
legal coverings. A copy of the column with all blanks filled with ship
parts (represented by Y) is made. Each time a legal covering is found,
for each square in the covering that has water, the corresponding Y in
the copy is changed to water. For each square of the copy that is
still marked with Y at the end, that means the square is a ship
part in all legal coverings, so the corresponding square of the actual
column is marked with an X, indicating a ship part. A second copy of
the column is made and handled similarly for water.

For each pair of blanks in the column, this makes a covering by making a
copy of the column with water in the position of those blanks and filling
the other blanks of the copy with ship parts.  Then it tests whether
the result is legal.

The test_array is made with extra squares at both ends filled with
water so that checking one square beyond the ends will work. The test
pointer is set to point to the second character of the array, which is
the beginning of the copy of the column being tested.
...
int try_full_but_two_col(  /* ARGUMENTS       */
 int col,                  /* column to try   */
 struct bap_world * world) /* puzzle model    */
{
  const char * name SET_TO "try_full_but_two_col";
  char test_array[MAX_TEST];     /* array containing test column           */
  char * test;                   /* test column                            */
  char test2S[MAX_NUMBER_ROWS];  /* copy of column for always ship part    */
  char test2W[MAX_NUMBER_ROWS];  /* copy of column for always water        */
  int n;                         /* row index, reused                      */
  int rows;                      /* number of rows in puzzle               */
  int row1;                      /* first test row to put water in         */
  int row2;                      /* second test row to put water in        */
  int filled_one;                /* flag indicating square set in column   */
  int is_legal;                  /* flag set by legal_col, 1=col is OK     */
  int found_legal;               /* flag set to 1 if legal column is found */
  char setting;                  /* character in a square                  */
  ...
  for (n SET_TO 0; n < rows; n++)
    {
      setting SET_TO square(n, col, world);
      test2W[n] SET_TO ((setting IS ' ') ? 'W' : setting);
      test2S[n] SET_TO ((setting IS ' ') ? 'Y' : setting);
    }
  for (row1 SET_TO 0; row1 < (rows - 1); row1++)
    {
      if (world->squares[row1][col] ISNT ' ')
	continue;
      for (row2 SET_TO (row1 + 1); row2 < rows; row2++)
	{
	  if (world->squares[row2][col] ISNT ' ')
	    continue;
	  for (n SET_TO 0; n < rows; n++)
	    {
	      setting SET_TO square(n, col, world);
	      test[n] SET_TO
		(((n IS row1) OR (n IS row2)) ? '.' :
		 (setting IS ' ') ? 'Y' : setting);
	    }
	  IFF(legal_col(test, col, &is_legal, world));
	  if (is_legal)
	    {
	      ...
	      for (n SET_TO 0; n < rows; n++)
		{
		  if ((test2W[n] IS 'W') AND (test[n] IS 'Y'))
		    test2W[n] SET_TO 'Y';
		  else if ((test2S[n] IS 'Y') AND (test[n] IS '.'))
		    test2S[n] SET_TO '.';
		}
	    }
	}
...
Side Effects: This is called if all the blanks but two in a row must be
filled with ship parts. The remaining two must be filled with water.

The idea is to find if any blank squares of the row are ship in all
legal coverings of the row and if any blank squares are water in all
legal coverings. A copy of the row with all blanks filled with ship
parts (represented by Y) is made. Each time a legal covering is found,
for each square in the covering that has water, the corresponding Y in
the copy is changed to water. For each square of the copy that is
still marked with Y at the end, that means the square is a ship
part in all legal coverings, so the corresponding square of the actual
row is marked with an X, indicating a ship part. A second copy of
the row is made and handled similarly for water.

For each pair of blanks in the row, this makes a covering by making a
copy of the row with water in the position of those blanks and filling
the other blanks of the copy with ship parts.  Then it tests whether
the result is legal.
...
water so that checking one square beyond the ends will work. The test
pointer is set to point to the second character of the array, which is
the beginning of the copy of the row being tested.

*/

int try_full_but_two_row(  /* ARGUMENTS    */
 int row,                  /* row to try   */
 struct bap_world * world) /* puzzle model */
{
  const char * name SET_TO "try_full_but_two_row";
  char test_array[MAX_TEST];     /* array containing test row           */
  char * test;                   /* test row                            */
  char test2S[MAX_NUMBER_COLS];  /* copy of row for always ship part    */
  char test2W[MAX_NUMBER_COLS];  /* copy of row for always water        */
  int n;                         /* column index, reused                */
  int cols;                      /* number of columns in puzzle         */
  int col1;                      /* first test column to put water in   */
  int col2;                      /* second test column to put water in  */
  int filled_one;                /* flag indicating square set in row   */   
  int is_legal;                  /* flag set by legal_row, 1=row is OK  */
  int found_legal;               /* flag set to 1 if legal row is found */
  char setting;                  /* character in a square               */
  ...
  for (col1 SET_TO 0; col1 < (cols - 1); col1++)
    {
      if (world->squares[row][col1] ISNT ' ')
	continue;
      for (col2 SET_TO (col1 + 1); col2 < cols; col2++)
	{
	  if (world->squares[row][col2] ISNT ' ')
	    continue;
	  for (n SET_TO 0; n < cols; n++)
	    {
	      setting SET_TO square(row, n, world);
	      test[n] SET_TO
		(((n IS col1) OR (n IS col2)) ? '.' :
		 (setting IS ' ') ? 'Y' : setting);
	    }
	  IFF(legal_row(test, row, &is_legal, world));
	  if (is_legal)
  ...
  for (n SET_TO 0; n < cols; n++)
    {
      if (test2S[n] IS 'Y')
	{
	  IFF(enx_square(row, n, world));
	  filled_one SET_TO 1;
	}
      else if (test2W[n] IS 'W')
	{
	  IFF(make_water(row, n, world));
	  filled_one SET_TO 1;
	}
    }
  if (filled_one)
  ...
  if (cover IS 0)
    {
      ...
      world->rows_in[0] SET_TO row;
      world->cols_in[0] SET_TO col;
      IFF(record_progress("blank cannot be ship",
			  "fill blank with water at crossing of", world));
    }
  return OK;
}
